...
from spectral_velocity_predictor_models import PhaseCorrelation, OFVelocityExtractor
...
class SpecFERNN_Cell(nn.Module):
    ...
    def forward(self, f, h, u):
        ...
        encoded_f = encoded_f.unsqueeze(1)                  
        encoded_f = encoded_f.expand(-1, n_modes, -1, -1, -1) # (B, n_modes, C, H, W)
...
class SpecSeq2SeqFERNN(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels, height, width,
                 ...
                 decoder_conv_layers=1, n_modes= 2, subpixel=False, periodic_bc=True,pool_type='max', velocity_predictor='PhaseCorrelation'):
        super().__init__()
        self.height = height
        self.width = width
        ...
        if velocity_predictor == 'PhaseCorrelation':
            # velocity predictor 
            self.velocity_predictor = PhaseCorrelation(n_modes=self.n_modes, 
                                                    subpixel=self.subpixel,
                                                    periodic_bc=self.periodic_bc
                                                    )
        elif velocity_predictor == 'OFVelocityExtractor':
            self.velocity_predictor = OFVelocityExtractor(n_modes=self.n_modes,
                                                          periodic_bc=self.periodic_bc)
    ...
    def forward(self, input_seq, pred_len, teacher_forcing_ratio=0.0, target_seq=None):
        ...
        with torch.no_grad():
            velocities, Probabilites = self.velocity_predictor(input_seq)  # (B, n_modes, 2)

        # initialize hidden state (B, n_modes, hidden_channels, H, W)
        h = torch.zeros(
        ...
        for t in range(pred_len):
            ...
            h = self.cell(current_frame, h, velocities)
